...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv7_upsampled = tf.layers.conv2d_transpose(conv7_1x1, num_classes, 4, strides=2, padding='same',
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
    ...
    conv4_1x1 = tf.layers.conv2d(pool4_out_scaled, num_classes, 1, strides=(1,1), padding='same',
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
    output = tf.add(conv7_upsampled, conv4_1x1)
    conv4_upsampled = tf.layers.conv2d_transpose(output, num_classes, 4, strides=(2, 2), padding='same',
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
    ...
    output = tf.add(conv4_upsampled, conv3_1x1)
    conv3_upsampled = tf.layers.conv2d_transpose(output, num_classes, 16, strides=(8, 8), padding='same',
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))

    return conv3_upsampled
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=correct_label, logits=logits))
...
def run():
    ...
    with tf.Session(config=config) as sess:
        ...
        train_nn(sess, 30, 20, get_batches_fn, train_op,
                 cross_entropy_loss, image_input, correct_label, keep_prob, learning_rate)
